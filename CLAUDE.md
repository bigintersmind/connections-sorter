...
- **A committed drop settles; it doesn't snap or glide home** (#23). Tile identity is positional, so the swap hands each of the two elements the *other's* word — left alone, the element under the finger kept its translate, lost `.tile-dragging`'s `transition: none`, and glided back to its own cell carrying the wrong word. So a drop is a FLIP, held in a `settle` state beside `drag`, in two phases. *Seeded*: both tiles render `settleTransforms(...)` — the positions their new words occupied on the last held frame, from the cell rects the drag measured and the release event's travel — wearing the looks those words had, under `.tile-seeded`'s `transition: none`; set in the same handler as `commitSwap` and `setSelected(null)`, so the commit frame is a pure re-seat. *Unseeded*: seeds and looks drop and `.tile`'s 0.15s transform transition carries each tile to rest, both cells lifted by `.tile-settling` for `SETTLE_MS`. The unseed is a `requestAnimationFrame` from a `useLayoutEffect` on `settle`, and that timing is load-bearing: React flushes the update in a later task, after the seeded frame has been through style, so the transition has a before-change style to travel from. Under `prefers-reduced-motion: reduce` nothing is seeded — the drop's own frame is already at rest. The `[activeKey, screen]` layout effect clears `settle` alongside `drag`; the tap path never sets it. A press during the glide targets the travelling tile, not the cell under it (#24).
