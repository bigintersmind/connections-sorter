...
- **First pointer wins.** `handleTilePointerDown` ignores a press while one is live, so a second finger or a resting palm can't replace `dragRef`, strand the first tile mid-air, and slip a stray selection through its trailing click. Multi-tile drags are out of scope.
