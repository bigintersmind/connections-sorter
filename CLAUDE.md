...
- **`--text-faint` is decoration and disabled states only** (it's below 4.5:1); running text, hints, placeholders and the footer use `--text-muted`. A field's edge is `--input-border` (the label inputs, the textarea), which clears 3:1 against both the page and the field's own fill (WCAG 1.4.11); `--border` and `--border-strong` are decorative hairlines and never a control's boundary. Never put an inline `outline: none` on a field — the shared `:focus-visible` ring in `index.css` is the only focus treatment, and an inline value would beat it.
