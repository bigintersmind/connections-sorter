...
- **Tile rects are measured once, on the frame the drag starts,** in *page* coordinates (`toPageRect`) and **on the cell (`.reveal`), never the tile**: the two boxes coincide, but a tile can be mid-transform at that moment (the press's own `:active` scale, a hover lift, the settle of a drop 100ms earlier still gliding) and the cell never is. Nothing moves during a drag (the carried tile rides on a transform, so no layout and no `ResizeObserver` pass), and page coordinates survive a scroll partway through. Don't re-measure per move — that buys a forced layout a frame.
...
- **A committed drop settles; it doesn't snap or glide home** (#23). Tile identity is positional, so the swap hands each of the two elements the *other's* word — left alone, the element under the finger kept its translate, lost `.tile-dragging`'s `transition: none`, and glided back to its own cell carrying the wrong word. So a drop is a FLIP, held in a `settle` state beside `drag`, in three phases (`settle.phase`). *Seeded*, one frame: both tiles render `settleTransforms(...)` — the positions their new words occupied on the last held frame, from the cell rects the drag measured and the release event's travel — wearing the looks those words had, under `.tile-seeded`'s `transition: none`; set in the same handler as `commitSwap` and `setSelected(null)`, so the commit frame is a pure re-seat. *Gliding*, `SETTLE_GLIDE_MS` (150): seeds and looks drop and `.tile`'s 0.15s transform transition carries each tile to rest, both cells lifted by `.tile-settling`. *Landed*, the rest of `SETTLE_MS` (320): the lift alone, held while the 0.3s box-shadows finish fading; then `settle` clears to null. Both constants live in `dragSwap.js` beside the drag scales, and `dragSwap.test.js` reads `index.css` to check they still match the transition they're timing. The unseed is a `requestAnimationFrame` from a `useLayoutEffect` on `settle`, and that timing is load-bearing: React flushes the update in a later task, after the seeded frame has been through style, so the transition has a before-change style to travel from; the two later steps are `setTimeout`s sharing the one slot on `settleTimersRef` (only ever one is pending), each guarded on the settle's identity so an older step can't advance a newer settle. Under `prefers-reduced-motion: reduce` nothing is seeded — the drop's own frame is already at rest. The `[activeKey, screen]` layout effect clears `settle` alongside `drag`; the tap path never sets it. A press during the glide falls through the displaced tile to the cell it is crossing: hit-testing follows the stacking order, so while that tile is in transit it wears `.tile-crossing` (`pointer-events: none`) and the press lands on the tile resting underneath (except over the vacated cell itself, which holds only the crossing tile, so a press there does nothing until *landed*) — bounded to the glide and not to the lift, so grabbing the tile the moment it comes to rest in the vacated cell still works (#24). The arriving tile stays live throughout, which leaves one accepted gap: grabbed by one edge and released at the target's far edge, it overhangs a neighbouring cell at the start of the glide (shrinking fast under the ease, but not fully clear until the glide ends at `SETTLE_GLIDE_MS`), so a press on that neighbour before it has pulled clear hits it instead — the price of being able to pick the dropped tile straight back up where the finger left it.
...
- **Lifting the carried tile takes a z-index on the CELL** (`.reveal:has(.tile-dragging)`), not just on the tile. `.reveal`'s entrance animates opacity and transform with a `both` fill, so every cell keeps a stacking context of its own for good, and a z-index inside one only sorts against that cell. The settle needs both cells lifted for its whole span, glide and landed alike (`.tile-settling` + `.reveal:has(.tile-settling)`), with the arriving — dropped — tile's cell at 3 (`.reveal:has(.tile-arriving)`, same specificity, so it must stay after) so it crosses over the tile it displaced whatever their DOM order. That lift is a *separate class* from `.tile-seeded`'s `transition: none`, which must last exactly one frame: fold them together and either the seed animates or the settle paints under its neighbours.
- **The carried tile's 1.04 and the drop target's 1.06 + 3px ring are a matched pair.** The two scales are `DRAG_LIFT_SCALE` / `DROP_TARGET_SCALE` in `dragSwap.js`, read by both the live drag's `liftTransform` and the settle seeds so the two can't drift apart. Grab a tile by its centre and it lands exactly over the target, so the ring reads only while the target's ringed box stays wider than the tile on top of it (5.4px past a resting edge — a clear halo, still inside the 6px grid gap). The carried tile is ringed in `--selected-text`, the *contrasting* token, for the same reason: ringed in `--selected-ring` like the target, the two merge into one dark blob.
...
- **Toggles expose state, not glyphs**: tiles and lock buttons carry `aria-pressed`; the lock check/ring, the hint arrow and the other icons are inline SVGs from `src/icons.jsx`, each `aria-hidden` (they were text glyphs once, and rendered from whatever font the OS fell back to). Tiles in a locked row are `aria-disabled` (still focusable so the words stay readable), not `disabled`; `.tile[aria-disabled="true"]` in `index.css` owns their cursor and suppresses the hover lift.
- **`--text-faint` is decoration and disabled states only** (below 4.5:1 everywhere in light and on `--surface` in dark, and only just over the line on the dark page at 4.53:1); running text, hints, placeholders and the footer use `--text-muted`. A field's edge is `--input-border` (the label inputs, the textarea), which clears 3:1 against both the page and the field's own fill (WCAG 1.4.11) — except a locked row's label input, which takes the row pastel as its edge and sits under 3:1 in light (#22 tracks whether to change that). `--border`/`--border-strong` carry hairlines and bordered-button edges (the small header buttons, Back, the unlocked lock chip, the switcher pill) but never a *field's* edge. Never put an inline `outline: none` on a field — the shared `:focus-visible` ring in `index.css` is the only focus treatment, and an inline value would beat it.
